...
export default function App() {
  ...
  return (
    <div className="min-h-screen bg-[radial-gradient(ellipse_at_top,_var(--tw-gradient-stops))] from-indigo-900 via-gray-900 to-black text-white font-sans selection:bg-indigo-500/30">
      <div className="max-w-3xl mx-auto p-4 sm:p-6 lg:p-8 min-h-screen flex flex-col">
        ...
        <header className="flex items-center justify-between py-6 mb-8">
          ...
          {account && (
            <button 
              onClick={logout}
              className="text-sm font-medium text-indigo-200 hover:text-white flex items-center gap-2 transition-colors"
            >
          ...
          {!account ? (
            ...
            <div className="flex-1 flex flex-col items-center justify-center max-w-md mx-auto w-full">
              <div className="text-center mb-10">
                ...
                <p className="text-indigo-200/80 text-lg">Buat alamat email sementara yang aman dan instan.</p>
              </div>

              <div className="w-full bg-white/5 backdrop-blur-xl border border-white/10 rounded-3xl p-6 sm:p-8 shadow-2xl">
                <form onSubmit={handleGenerate} className="space-y-5">
                  <div>
                    <label className="block text-sm font-medium text-indigo-200 mb-2">Custom Name</label>
                    <input 
                      type="text" 
                      required
                      value={username}
                      ...
                      className="w-full bg-black/20 border border-white/10 rounded-xl px-4 py-3 text-white placeholder:text-white/20 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-transparent transition-all"
                    />
                  </div>
                  <div>
                    <label className="block text-sm font-medium text-indigo-200 mb-2">Pilih Domain</label>
                  ...
                  <button 
                    ...
                    className="w-full bg-gradient-to-r from-indigo-500 to-purple-600 hover:from-indigo-400 hover:to-purple-500 text-white font-semibold py-3.5 px-4 rounded-xl shadow-lg shadow-indigo-500/25 transition-all disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2"
                  >
            ...
            <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
              ...
              <div className="bg-white/5 backdrop-blur-xl border border-white/10 rounded-3xl p-6 sm:p-8 shadow-2xl relative overflow-hidden group">
                ...
                <p className="text-sm font-medium text-indigo-200 mb-3">Alamat Email Anda</p>
                <div 
                  ...
                  {messages.length === 0 ? (
                    <div className="flex flex-col items-center justify-center h-64 text-center px-4">
                      <div className="w-16 h-16 rounded-full bg-white/5 flex items-center justify-center mb-4">
                        <Loader2 className="w-8 h-8 text-indigo-400/50 animate-spin" />
                      </div>
                      <p className="text-indigo-200 font-medium">Menunggu email masuk...</p>
                      <p className="text-sm text-indigo-200/60 mt-2">Otomatis refresh setiap 10 detik</p>
                    </div>
                  ) : (
                          ...
                          >
                            <div className="flex-1 min-w-0">
                              <div className="flex items-center justify-between mb-1">
                                ...
                                <span className="text-xs text-indigo-200/60 whitespace-nowrap">
                                  {new Date(msg.createdAt).toLocaleTimeString([], {hour: '2-digit', minute:'2-digit'})}
                                </span>
                              </div>
                              <p className="text-sm text-indigo-200 truncate">{msg.subject || '(Tanpa Subjek)'}</p>
                            </div>
                          </button>
                        </li>
                      ))}
              ...
              {messageLoading || selectedMessage.loading ? (
                ...
                <div className="prose prose-invert max-w-none">
                  {selectedMessage.html ? (
                    ...
                    <pre className="whitespace-pre-wrap font-sans text-indigo-100/80 text-sm leading-relaxed">
